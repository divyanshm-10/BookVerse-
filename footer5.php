...
<title>Dim Cinematic Bookstore Footer</title>
<style>
  body {
    margin: 0;
    font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    background: #121212; /* dim background */
    overflow-x: hidden;
    cursor: none;
    color: #eee;
  }
  ...
  footer {
    ...
    animation: neonBorder 4s linear infinite alternate;
    border-radius: 20px;
  }
...
<script>
  ...
  document.addEventListener('mousemove', e => {
    ...
    books.forEach(book => {
      ...
      const open = Math.sin(Date.now()/500)*15; // open like pages
      book.style.transform = `rotateX(${angleX + open}deg) rotateY(${angleY}deg) translateY(-100px)`;
    });

    const particleElements = document.querySelectorAll('.particle');
    particleElements.forEach(p => {
      const px = parseFloat(p.style.left) || Math.random()*100;
      const py = parseFloat(p.style.top) || Math.random()*100;
